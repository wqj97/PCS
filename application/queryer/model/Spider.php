...
class Spider extends SpiderSetting
{
    ...
    protected function save_into_db ($result, $id = 0, $type, $keyWord = '', $city)
    {
        ...
        if ($from_id) {
            if ($type == 'Tbk') {
                $last_update_time = Db::query("SELECT unix_timestamp(P_last_update) AS 'last_update_time' FROM product_comparison WHERE P_Id = ? and {$type} IS NOT NULL 
ORDER BY P_last_update DESC LIMIT 1",
                    [$id, $city]);
            } else {
                $last_update_time = Db::query("SELECT unix_timestamp(P_last_update) AS 'last_update_time' FROM product_comparison WHERE P_Id = ? AND P_city = ? and {$type} IS NOT NULL 
ORDER BY P_last_update DESC LIMIT 1",
                    [$id, $city]);
            }
        } else {
            if ($type == 'Tbk') {
                $last_update_time = Db::query("SELECT unix_timestamp(P_last_update) AS 'last_update_time' FROM product_comparison WHERE P_keyWord = ? AND {$type} IS NOT NULL
ORDER BY P_last_update DESC LIMIT 1",
                    [$keyWord, $city]);
            } else {
                $last_update_time = Db::query("SELECT unix_timestamp(P_last_update) AS 'last_update_time' FROM product_comparison WHERE P_keyWord = ? AND P_city = ? AND {$type} IS NOT NULL
ORDER BY P_last_update DESC LIMIT 1",
                    [$keyWord, $city]);
            }
        }
